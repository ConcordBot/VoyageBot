...
class Welcome(commands.Cog):
    ...
    async def onjoin(self, member, server):
        # Welcome
        try:
            welcomeChannel = server.get_channel(int(self.settings.getServerStat(server, "WelcomeChannel")))
        except:
            welcomeChannel = None
        if welcomeChannel:
        ...
        if rules:
            try:
                await member.send(Utils.suppressed(server, "***{}*** **Rules:**:\n{}".format(server.name, rules)))
            except:
                pass
    ...
    async def setwelcome(self, ctx, *, message=None):
        ...
        try:
            welcomeChannel = ctx.guild.get_channel(int(self.settings.getServerStat(ctx.guild, "WelcomeChannel")))
        except:
            welcomeChannel = None
        if welcomeChannel:
    ...
    async def testwelcome(self, ctx, *, member=None):
        ...
        if message in (None, ""):
            return await ctx.send(
                'Welcome message not setup.  You can do so with the `{}setwelcome [message]` command.'.format(
                    ctx.prefix))
    ...
    async def setgoodbye(self, ctx, *, message=None):
        ...
        if welcomeChannel:
            ...
            if self._getDefault(ctx.guild):
                msg = 'The current goodbye channel is the default channel (**{}**).'.format(
                    self._getDefault(ctx.guild).mention)
            else:
    ...
    async def testgoodbye(self, ctx, *, member=None):
        ...
        if message in (None, ""):
            return await ctx.send(
                'Goodbye message not setup.  You can do so with the `{}setgoodbye [message]` command.'.format(
                    ctx.prefix))
    ...
    async def setwelcomechannel(self, ctx, *, channel: discord.TextChannel = None):
        ...
        if channel == None:
            ...
            if self._getDefault(ctx.guild):
                msg = 'Welcome and goodbye messages will be displayed in the default channel (**{}**).'.format(
                    self._getDefault(ctx.guild).mention)
            else:
