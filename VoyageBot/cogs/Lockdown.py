...
class Lockdown(commands.Cog):
    ...
    def _get_mention(self, channel, show_lock=False, lockdown_list=[]):
        ...
        if isinstance(channel, discord.CategoryChannel):
            synced = [x for x in channel.channels if x.permissions_synced] if hasattr(channel,
                                                                                      "permissions_synced") else channel.channels
        ...
        return "{}{} {}{}".format(lock_text, lock, "🔄 " if not hasattr(channel,
                                                                        "permissions_synced") or channel.permissions_synced else "🟦 " if channel.category else "",
                                  channel.mention)
    ...
    async def _perform_lockdown(self, ctx, target_channels, unlock=False):
        ...
        guild = ctx if isinstance(ctx, discord.Guild) else ctx.guild if isinstance(ctx,
                                                                                   discord.ext.commands.Context) else None
        ...
        while len(lockdown):
            ...
            if isinstance(channel, discord.CategoryChannel):
                ...
                new_chans = [x for x in channel.channels if not x in lockdown and (
                            not hasattr(channel, "permissions_synced") or not x.permissions_synced)]
                lockdown.extend(new_chans)
            else:
                channels += 1
                # Check if we even need to update perms based on syncing/category inclusion
                if channel.category and hasattr(channel,
                                                "permissions_synced") and channel.permissions_synced and channel.category in target_channels: continue
            ...
            if unlock:
                perm_check = any(x[0] in self.lockdown_perms and x[1] != None for x in overs)
            else:
                perm_check = not all([x == False for x in (overs.send_messages, overs.add_reactions, overs.speak)])
    ...
    async def listlock(self, ctx):
        ...
        await Message.EmbedText(title="Current Lockdown List - {:,} Total".format(len(lockdown)), description=desc,
                                color=ctx.author, footer=self.key).send(ctx)
    ...
    async def listlockall(self, ctx):
        ...
        if not await Utils.is_bot_admin_reply(ctx): return
        lockdown = self.settings.getServerStat(ctx.guild, "LockdownList", [])
        desc = "\n".join([self._get_mention(x, lockdown_list=lockdown, show_lock=True) for x in
                          self._order(ctx, ctx.guild.channels, only_id=False)])
        await Message.EmbedText(title="All Channel Lockdown Status - {:,} Total".format(len(ctx.guild.channels)),
                                description=desc, color=ctx.author, footer=self.key_long).send(ctx)
    ...
    async def addlock(self, ctx, *, channel_list=None):
        ...
        if not len(resolved): return await ctx.send(
        ...
        await Message.EmbedText(
            ...
            description=desc, color=ctx.author, footer=self.key).send(ctx)
    ...
    async def addlockall(self, ctx):
        ...
        await Message.EmbedText(title="{:,} New Entr{} Added to Lockdown List".format(len(new_lockdown), "y" if len(
            new_lockdown) == 1 else "ies"), description=desc, color=ctx.author, footer=self.key).send(ctx)
    ...
    async def remlock(self, ctx, *, channel_list=None):
        ...
        desc = "\n".join([self._get_mention(x) for x in resolved])
        await Message.EmbedText(
            title="{:,} Entr{} Removed from Lockdown List".format(len(resolved), "y" if len(resolved) == 1 else "ies"),
            description=desc, color=ctx.author, footer=self.key).send(ctx)
    ...
    async def remlockall(self, ctx):
        ...
        desc = "\n".join([self._get_mention(x) for x in channels])
        await Message.EmbedText(
            title="{:,} Entr{} Removed from Lockdown List".format(len(lockdown), "y" if len(lockdown) == 1 else "ies"),
            description=desc, color=ctx.author, footer=self.key).send(ctx)
    ...
    async def lockdown(self, ctx, target_channel=None):
        ...
        if cats == chans == 0: return await Message.EmbedText(title="Lockdown",
                                                              description="**LOCKDOWN FAILED!**").edit(ctx, message)
        await Message.EmbedText(
            title="Lockdown",
    ...
    async def unlockdown(self, ctx, target_channel=None):
        ...
        if cats == chans == 0: return await Message.EmbedText(title="Unlockdown",
    ...
    async def _anti_raid_respond(self, member):
        ...
        if response.lower() == "mute":
            ...
            try:
                await member.guild.ban(member, reason="Anti-raid active")
            except:
                pass
        else:
            try:
                await member.guild.kick(member, reason="Anti-raid active")  # Assume kick if nothing specified
            except:
                pass
                    ...
                    if m:  # Got a member or role - let's get the channel
                        ...
                        if channel:
                            ...
                            if c:  # Got a member/role and a channel - try to post the ping
                                try:
                                    await c.send("{} - Anti-raid has been enabled!".format(m.mention))
                                except:
                                    pass
            self.settings.setServerStat(member.guild, "AntiRaidLastJoin", time.time())
    ...
    @commands.command()
    async def antiraid(self, ctx, *, on_off=None, join_number=None, join_seconds=None, kick_ban_mute=None,
        ...
        if on_off == None:  # Gather settings and report the current
            ...
            if time.time() - ar_lastjoin >= cooldown_minutes: self.settings.setServerStat(ctx.guild, "AntiRaidActive",
                                                                                          False)  # No longer watching - disable anti-raid
    ...
    async def antiraidping(self, ctx, user_or_role=None, channel=None):
        ...
        if user_or_role == None:  # print the settings
            ...
            return await ctx.send("Anti-raid activity will mention {} and be announced in {}!".format(
                Nullify.escape_all(user_role.display_name) if isinstance(user_role,
                                                                         discord.Member) else Nullify.escape_all(
        ...
        return await ctx.send("Anti-raid activity will mention {} and be announced in {}!".format(
            Nullify.escape_all(ur.display_name) if isinstance(ur, discord.Member) else Nullify.escape_all(ur.name),
            ch.mention))
